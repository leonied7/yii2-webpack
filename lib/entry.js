...
module.exports = class Entry {
  constructor (entryName, {assetDirectory, webpackDirectory, template = resolve(__dirname, '../index.ejs'), ...options}) {
    this.entryName = entryName
    this.assetDirectory = this.constructor.correctDirectory(assetDirectory)
    this.webpackDirectory = this.constructor.correctDirectory(webpackDirectory)
  ...
  /**
   * используется внутри шаблона, формат app</webpackDirectory></assetDirectory>
   * пример: app/assets
   * @return {string}
   */
  get namespace () {
    let namespace = ['app', this.webpackDirectory, this.assetDirectory]
    _.pull(namespace, '')
  ...
  /**
   * формирует sourcePath формата @app</webpackDirectory>/<outputPath>
   * пример: @app/dist
   * @param outputPath
   * @return {string}
   */
  getSourcePath (outputPath) {
    return '@app' + this.constructor.addFirstSlash(this.webpackDirectory) +
      this.constructor.calculateDist(outputPath)
  }
